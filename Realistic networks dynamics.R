...
SIH_function<-function(dispV=NA,species=15,numCom=100){
  ...
  d<-shortest.paths(graph, mode="all", weights=NULL, algorithm="automatic")
  ...
  hold_data<-data.frame(L_SR=mean(apply(Abund>0,3,rowSums)),
               ...
               Base_growth=mean(Meta_dyn$Base_growth),
               Species_sorting=mean(Meta_dyn$Species_sorting),
               Mass_effects=mean(Meta_dyn$Mass_effects),
               Dispersal=dispersal)
  if(i==1){
    SIH_data<-hold_data
  } else {
